...
namespace MVCDemo.App_Start
{
    ...
    public static class NinjectWebCommon 
    {
        ...
        {
            if(ConfigurationManager.AppSettings["UseDatabase"].ToString().ToUpper()=="TRUE")
                kernel.Bind<IUnitOfWork>().To<UnitOfWork>().InRequestScope();
            else
                kernel.Bind<IUnitOfWork>().To<UnitOfWorkFake>().InRequestScope();

            kernel.Bind<DemographicService>().To<DemographicService>().InRequestScope();
            kernel.Bind<StateService>().To<StateService>().InRequestScope();
        }        
    }
}
